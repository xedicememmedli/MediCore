...
const mockData = [
  { id: 1, name: "Antibiotiklər", description: "Bakteriyalara qarşı dərmanlar", iconPath: "💊" },
  { id: 2, name: "Ağrıkəsicilər", description: "Ağrı və qızdırmanı azaldan dərmanlar", iconPath: "🩹" },
  { id: 3, name: "Vitaminlər", description: "Vitamin və mineral əlavələri", iconPath: "🌿" },
  { id: 4, name: "Kardioloji", description: "Ürək-damar sistemi üçün dərmanlar", iconPath: "❤️" },
  { id: 5, name: "Dermatoloji", description: "Dəri xəstəlikləri üçün dərmanlar", iconPath: "✨" },
];
...
const styles = `
  ...
  .cat-name { font-size: 15px; font-weight: 800; color: #154360; margin-bottom: 4px; }
  .cat-desc { font-size: 12px; color: #5D8AA8; line-height: 1.5; }
  .cat-actions {
  ...
  .icon-opt:hover { border-color: #AED6F1; background: #EBF5FB; }
  .icon-opt.selected { border-color: #1F618D; background: #EBF5FB; box-shadow: 0 0 0 3px rgba(31,97,141,0.1); }
...
const emptyForm = { name: "", description: "", iconPath: "💊" };
...
export default function CategoryPage() {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const res = await axios.get(`${API}/api/Category`, { headers: authHeader() });
  ...
  const openEdit = (item) => {
    setEditItem(item);
    setForm({ name: item.name || "", description: item.description || "", iconPath: item.iconPath || "💊" });
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      fd.append("IconPath", form.iconPath);
      if (editItem) {
        fd.append("Id", editItem.id);
        await axios.put(`${API}/api/Category`, fd, { headers: authHeader() });
      } else {
        await axios.post(`${API}/api/Category`, fd, { headers: authHeader() });
      }
      await fetchData();
      closeModal();
    } catch (err) {
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true);
    try {
      await axios.delete(`${API}/api/Category/SoftDelete/${deleteTarget.id}`, { headers: authHeader() });
      await fetchData();
  ...
  return (
    <>
      ...
      <div className="pg-header">
        <div>
          <h1>Kateqoriyalar <span className="count-badge">{filtered.length}</span></h1>
          <p>Dərman kateqoriyalarını idarə edin</p>
        </div>
        <button className="btn-primary" onClick={openAdd}>＋ Yeni Kateqoriya</button>
      </div>

      <div className="search-bar">
        <span style={{ color: "#B0C4D8", fontSize: 14 }}>🔍</span>
        <input placeholder="Kateqoriya axtar..." value={search} onChange={e => setSearch(e.target.value)} />
      </div>
      ...
      {loading ? (
        ...
        <div className="empty-state">
          ...
          <p>Yeni kateqoriya əlavə etmək üçün yuxarıdakı düyməyə klikləyin</p>
        </div>
      ) : (
          ...
          {filtered.map(item => (
            <div className="cat-card" key={item.id}>
              <div className="cat-card-top">
                <div className="cat-icon">{item.iconPath || "💊"}</div>
                <div>
                  <div className="cat-name">{item.name}</div>
                  <div className="cat-desc">{item.description || "Açıqlama yoxdur"}</div>
                </div>
              </div>
        ...
        <div className="overlay" onClick={e => e.target === e.currentTarget && closeModal()}>
          <div className="modal">
            <div className="modal-head">
              <div className="modal-title">{editItem ? "✏️ Kateqoriyanı Redaktə Et" : "＋ Yeni Kateqoriya"}</div>
            ...
            <div className="modal-body">
              ...
              <div className="fg">
                ...
                <input className="fi" placeholder="Məs: Antibiotiklər" value={form.name}
                  onChange={e => set("name", e.target.value)} autoFocus />
              </div>
              <div className="fg">
                <label className="fl">Açıqlama</label>
                <textarea className="fi" rows={3} placeholder="Kateqoriya haqqında məlumat..."
                  value={form.description} onChange={e => set("description", e.target.value)} />
              </div>
              <div className="fg">
                <label className="fl">İkon seçin</label>
                <div className="icon-picker">
                  {iconOptions.map(ic => (
                    <div key={ic} className={`icon-opt ${form.iconPath === ic ? "selected" : ""}`}
                      onClick={() => set("iconPath", ic)}>{ic}</div>
                  ))}
                </div>
              </div>
            </div>
      ...
      {deleteTarget && (
        <div className="overlay" onClick={e => e.target === e.currentTarget && setDeleteTarget(null)}>
          <div className="confirm-box">
            ...
            <h3>Silinsin?</h3>
            <p><strong>"{deleteTarget.name}"</strong> kateqoriyası silinəcək.<br />Bu əməliyyat geri alına bilər.</p>
